...
class Test_Positive:
    def test_1_find_text_in_command(self, make_folders, make_files, print_time):
        assert find_text_in_command(f'cd {data["folder_in"]}; '
    ...
    def test_2_find_text_in_command(self, print_time):
        assert find_text_in_command(f'ls {data["folder_out"]}',
                                    f'{data["filename"]}'), 'Test2 FAIL'

    def test_3_find_text_in_command(self, clear_folders, print_time):
        assert find_text_in_command(f'crc32 {data["folder_out"]}/*',
                                    crc.crc32(f'{data["folder_out"]}/{data["filename"]}.{data["typearch"]}').lower()), \
            'Test3 FAIL'