import pytest
from dz4 import find_text_in_command, getout
from task import ssh_find_text_in_command, ssh_getout
import yaml
import random
import string
...
def make_folders_local():
    return find_text_in_command(f'mkdir {data["folder_out_local"]}', "")
...
def make_folders():
    return ssh_find_text_in_command(host=str(data["host"]),
                                    ...
                                    command=f'mkdir {data["folder_in"]} {data["folder_out"]}',
                                    text="")
...
def clear_folders():
    ...
    ssh_find_text_in_command(host=str(data["host"]),
                             user=str(data["user"]),
                             password=str(data["password"]),
                             command=f'rm -rf {data["folder_in"]} {data["folder_out"]}',
                             text="")

@pytest.fixture()
def clear_folders_local():
    yield
    find_text_in_command(f'rm -rf {data["folder_out_local"]}', "")
